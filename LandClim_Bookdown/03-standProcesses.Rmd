...
-   Available **light at forest floor** is higher than a
    species-specific threshold value ($minL_s$);

-   **Winter temperature** (mean temperature of the coldest month) is
    higher than a threshold minimum temperature ($minT_s$);

-   The sum of **growing degree-days** exceeds the minimum
    species-specific requirement ($minDD_s$);

-   The **drought index** ($Dl_{se}$) has to be lower than the
    species-specific maximum drought tolerance;

-   **Browsing probability** has to be smaller than a uniformly
    distributed random number (see [browsing
    section](%5B#sectionBrowsing%5D(http://127.0.0.1:35763/#sectionBrowsing%20%22#sectionBrowsing%22))
    for more information about browsing).

-   A user defined **establishment coefficient** ($estCoeff_s$, defined
    in the landtype input file) has to be higher than a uniformly
    distributed random number. This variable represents additional
    factors which are not explicitly modelled.

-   If **pasture management** is turned on, then grazing intensity is
    also used (see section on [Pasture
    Management](%5B#sectionPastureManagement%5D(http://127.0.0.1:35763/#sectionPastureManagement%20%22#sectionPastureManagement%22)))
    with a random number generator to determine if establishment could
    occur this year. Pasture management adds an additional constraint
    that there needs to be a minimum of five good establishment years
    *in a row* for a cohort of that particular species to establish.
...
+===========================+===========+===========================+
| SaplingInitialBiomass     | numeric   | 0,01 tonnes (10kg) the    |
|                           |           | initial tree biomass (Bi  |
|                           |           | (t=0) assumed to be the   |
|                           |           | same for all the tree     |
|                           |           | species. This value is    |
|                           |           | fixed in the code         |
|                           |           |                           |
|                           |           | \_ initialSaplingBiomass  |
+---------------------------+-----------+---------------------------+
| Rmax                      | numeric   | User defined species      |
|                           |           | specific parameter that   |
|                           |           | shows the maximum growth  |
|                           |           | rate under optimum        |
|                           |           | environmental conditions  |
|                           |           | per year                  |
+---------------------------+-----------+---------------------------+
| Sapling                   | category  | Defined as trees less     |
|                           |           | than 10 years old         |
+---------------------------+-----------+---------------------------+
| rela                      |           | Ratio of realized growth  |
| tiveGrowthPerformance     |           | incremetn during the last |
|                           |           | 10 years to max potential |
|                           |           | growth increment under    |
|                           |           | optimal environmental     |
|                           |           | conditions (aka species   |
|                           |           | with better grotwht are   |
|                           |           | capable of establishment  |
|                           |           | more trees)               |
|                           |           |                           |
|                           |           | \_relativ                 |
|                           |           | eGrowthPerformance\[i\] = |
|                           |           | (\_init                   |
|                           |           | ialRealizedBiomass\[i\] - |
|                           |           | \_init                    |
|                           |           | ialSaplingBiomass\[i\]) / |
|                           |           | \_optimum                 |
|                           |           | GrowthPerformance\[i\];   |
+---------------------------+-----------+---------------------------+
| \_opt                     |           | maximun potential growth  |
| imumGrowthPerformance     |           | icrement under optimal    |
|                           |           | environmental conditions  |
|                           |           |                           |
|                           |           | \_initialSapl             |
|                           |           | ingBiomass\[speciesId\]   |
|                           |           | \* (float)Math.Pow(1.0f + |
|                           |           | \_Rmax\[speciesId\], \_e  |
|                           |           | stablishmentInterval) -   |
|                           |           | \_initialSapli            |
|                           |           | ngBiomass\[speciesId\];   |
+---------------------------+-----------+---------------------------+
|                           |           |                           |
...
| Vari able name              | T y p e           | Location                                 | R ef er en ce        | Description                                                                                                                                  |
|-----------------------------|-------------------|------------------------------------------|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------|
| Grow thTr ee()              | F u n c t i o n   | Cohort.cs                                | Sc hu ma ch er 20 04 | The realized growth rate $r_i(t)$                                                                                                            |
| Sp ecie sPar amet ers. RMax | P a r a m e t e r | User input in the species parameter file |                      | Optimun growth rate $r_s$                                                                                                                    |
| Delt aBio mass              | V a l u e         | float C ohort.Del taBiomass              |                      | the change of individual tree biomass per year                                                                                               |
| Bio mass                    | V a l u e         | float Cohor t.Biomass                    |                      | The biomass of an individual tree in a cohort (multiplied by the number of trees in the cohort Nc will provide the biomass in of the cohort) |
| Upda teDi amet er()         | F u n c t i o n   |                                          |                      | This function transforms the biomass into dbh values                                                                                         |
